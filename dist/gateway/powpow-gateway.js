...
import { extractMessageContent, normalizeDbRow, normalizeHistoryMessage, } from "../messaging/inbound-handler.js";
import { sendReply } from "../messaging/send-service.js";
const powpowIngressIdentity = {
    key: "powpow-user-id",
    normalizeEntry: (entry) => entry.trim(),
    normalizeSubject: (id) => id.trim(),
    sensitivity: "pii",
    entryIdPrefix: "powpow-entry",
};
export async function startPowpowGatewayAccount(ctx) {
    const account = ctx.account;
    ctx.setStatus({
        accountId: account.accountId,
        lifecycle: "starting",
    });
    ctx.log?.info?.(`[${account.accountId}] starting PowPow channel (digital human: ${account.digitalHumanId})`);
    if (!account.configured) {
        throw new Error("PowPow digital human id and webhook token are required");
    }
    const channelRuntime = ctx.channelRuntime;
    if (!channelRuntime?.inbound?.buildContext) {
        throw new Error("PowPow requires its registered channel runtime context builder");
    }
    ...
    const resolveInboundAccess = async (senderId, sessionId, contextBinding) => await resolveStableChannelMessageIngress({
        channelId: "powpow",
        accountId: account.accountId,
    ...
    const dispatchInbound = async (inbound) => {
        const resolvedAccess = await resolveInboundAccess(inbound.senderId, inbound.sessionId);
        if (resolvedAccess.senderAccess.decision !== "allow") {
            ctx.log?.warn?.(`[${account.accountId}] dropping PowPow message from ${inbound.senderId} (${resolvedAccess.senderAccess.reasonCode})`);
            return;
        }
        const bodyForAgent = extractMessageContent(inbound.contentType, inbound.content);
        await dispatchInboundDirectDm({
            channelRuntime,
            resolveChannelIngress: async (contextBinding) => {
                const exactAccess = await resolveInboundAccess(inbound.senderId, inbound.sessionId, contextBinding);
                if (!exactAccess.senderAccess.allowed) {
                    throw new Error(`PowPow sender authorization changed before dispatch (${inbound.senderId})`);
                }
                return exactAccess;
            },
            cfg: ctx.cfg,
            ...
            onRecordError: (err) => {
                ctx.log?.error?.(`[${account.accountId}] failed recording PowPow inbound session: ${String(err)}`);
            },
            onDispatchError: (err, info) => {
                ctx.log?.error?.(`[${account.accountId}] PowPow ${info.kind} reply failed: ${String(err)}`);
            },
        });
    };
    let inboundQueue = [];
    ...
    const handleInbound = (inbound) => {
        if (!inbound)
            return;
        if (!dedup.markSeen(inbound.messageId))
            return;
        inboundQueue.push(inbound);
        void processQueue().catch((err) => {
            ctx.log?.error?.(`[${account.accountId}] PowPow inbound dispatch failed: ${String(err)}`);
        });
    };
        ...
        start: async () => {
            const poller = pollEnabled
                ? new HistoryPoller({
                    ...
                    onError: (error) => {
                        ctx.log?.debug?.(`[${account.accountId}] history poll failed: ${String(error)}`);
                    },
                })
                : null;
            ...
            ctx.log?.info?.(`[${account.accountId}] PowPow channel started (polling: ${pollEnabled ? `on @ ${pollIntervalMs}ms` : "off"}, realtime: ${realtimeEnabled ? "on" : "off"})`);
            return {
                stop: async () => {
                    inboundQueue = [];
                    poller?.stop();
...
//# sourceMappingURL=powpow-gateway.js.map