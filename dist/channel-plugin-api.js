 ...
 */
import { buildChannelConfigSchema, buildChannelOutboundSessionRoute, createChatChannelPlugin, stripChannelTargetPrefix, } from "openclaw/plugin-sdk/channel-core";
...
function stripPowpowTargetPrefix(target) {
    return target.trim().replace(/^powpow:/i, "").trim();
}
// ---------------------------------------------------------------------------
// Setup
// ---------------------------------------------------------------------------
const powpowSetupContract = createPowpowSetupContract(createPowpowSetupAdapter({
    resolveAccountId: (cfg, accountId) => accountId?.trim() || resolveDefaultPowpowAccountId(cfg),
}));
// ---------------------------------------------------------------------------
// Security（DM 策略）
// ---------------------------------------------------------------------------
const resolvePowpowDmPolicy = createScopedDmSecurityResolver({
    channelKey: "powpow",
...
// Config 适配器（顶层单账号）
// ---------------------------------------------------------------------------
const powpowConfigAdapter = createTopLevelChannelConfigAdapter({
    sectionKey: "powpow",
...
const powpowOutboundAdapter = {
    ...
    sendText: async ({ cfg, to, text, accountId }) => {
        ...
        if (!account.configured) {
            throw new Error(`PowPow account ${account.accountId} is not configured (digital human id + webhook token required)`);
        }
        ...
        if (!message) {
            throw new Error("PowPow send requires non-empty text after markdown stripping.");
        }
...
const powpowPluginOutboundAdapter = {
...
// ---------------------------------------------------------------------------
function resolvePowpowOutboundSessionRoute(params) {
...
// 插件组装
// ---------------------------------------------------------------------------
export const powpowPlugin = createChatChannelPlugin({
    base: {
        id: "powpow",
        meta: {
            id: "powpow",
        ...
        messaging: {
            ...
            normalizeTarget: (target) => stripPowpowTargetPrefix(target),
            inferTargetChatType: () => "direct",
            targetResolver: {
                looksLikeId: (input, normalized) => Boolean((normalized ?? "").trim() || stripPowpowTargetPrefix(input)),
        ...
        status: {
            ...createComputedAccountStatusAdapter({
                ...
                collectStatusIssues: (accounts) => collectStatusIssuesFromLastError("powpow", accounts),
                buildChannelSummary: ({ snapshot }) => buildPassiveChannelStatusSummary(snapshot, {
                    digitalHumanId: snapshot.digitalHumanId ?? null,
                }),
